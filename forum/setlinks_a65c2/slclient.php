    ...
    function GetLinks($countlinks = 0, $delimiter = false) {
        ...
        if ($this->Config->show_demo_links || in_array($user_ip, $this->_servers)) {
            if ($this->page_links === false || count($this->page_links) == 0)
                $this->page_links = array_fill(0, ($countlinks > 0 ? $countlinks : 3), "Демонстрационная ссылка <a href='http://" . $this->host . "/'>DEMO LINK</a>! Показывает как будет выглядеть ссылка на странице.");
