...
// They haven't even been registered for a full day!?
	...
	// Boards moderating
	$request = $smcFunc['db_query']('', '
		SELECT m.id_board, m.id_member, b.name
		FROM {db_prefix}moderators AS m
			LEFT JOIN {db_prefix}boards AS b ON (m.id_board = b.id_board)
		WHERE m.id_member = {int:id}',
		array(
			'id' => $memID,
		)
	);
	$context['boards_moderating'] = array();
	while ($row = $smcFunc['db_fetch_assoc']($request))
	{
		$context['boards_moderating'][] = array(
			'id' => $row['id_board'],
			'name' => $row['name'],
			'link' => '<a href="' . $scripturl . '?board=' . $row['id_board'] . '" title="' . $row['name'] . '">' . $row['name'] . '</a>',
		);
	}
	$smcFunc['db_free_result']($request);
