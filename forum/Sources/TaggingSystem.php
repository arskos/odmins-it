...
function suggestTagsAjaxs()
{
	...
	$context['sub_template'] = 'tagging_suggest_ajax';
}
	...
	if (!empty($modSettings['tag_enabled']))
	{
		...
		if (!empty($_POST['tags_news']))
		{
			$request = $smcFunc['db_query']('', '
				...
				array(
					'tags_news' => $_POST['tags_news'],
				)
			);
			// Step 2: delete duplicate tags and add the new ones
			while ($row = $smcFunc['db_fetch_assoc']($request))
			{
				if (in_array($row['tag'],$_POST['tags_news']))
				{
					$deleteTag = array_search($row['tag'],$_POST['tags_news'],false);
					array_splice($_POST['tags_news'], $deleteTag, 1);
			...
			if (!empty($_POST['tags_news']))
			{
				foreach ($_POST['tags_news'] as $tag_name)
				{
					$smcFunc['db_insert']('',
					'{db_prefix}tags',
						array(
