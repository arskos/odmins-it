...
function Post()
{
	...
	// Drafts?
	$context['save_draft'] = allowedTo('post_draft') && !empty($modSettings['masterSaveDrafts']);
...
function Post2()
{
	...
	if (!empty($topic) && !isset($_REQUEST['msg']))
	{
		...
		// OK, first figure out from the lock *changes* what the state we're interested in is
		if (isset($_POST['lock']))
	...
	if (isset($_FILES['attachment']['name']) || (!empty($_SESSION['temp_attachments']) && empty($_POST['from_qr'])))
	{
		...
		foreach ($_FILES['attachment']['tmp_name'] as $n => $dummy)
		{
			...
			elseif (createAttachment($attachmentOptions))
			{
	...
	// Make the poll...
	if (!empty($torrents))
		$_POST['message'] .= '<br /><br />[torrent]' . implode(',', $torrents) . '[/torrent]';
	// Make the poll...
	if (isset($_REQUEST['poll']))
	{
		// Create the poll.
...
function saveDraft()
{
	global $context, $txt, $smcFunc, $topic, $board, $user_info, $modSettings;

	if (!isset($_REQUEST['draft']) || $user_info['is_guest'] || !allowedTo('post_draft') || empty($modSettings['masterSaveDrafts']))
		return false;
