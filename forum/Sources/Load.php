...
function loadUserSettings()
{
	...
	// Detect user for tracker / download torrent pages by passkey (if it's enabled)
	elseif ((defined('IN_TRACKER') && IN_TRACKER == true) || (isset($_REQUEST['action']) && $_REQUEST['action'] == 'tracker' && isset($_REQUEST['sa']) && $_REQUEST['sa'] == 'download'))
	{
		if (!empty($modSettings['enablePasskey']) && isset($_REQUEST['p']))
			$id_member = getUserByPasskey($_REQUEST['p']);
		else
			$id_member = getUserByIP();
...
function loadMemberData($users, $is_name = false, $set = 'normal')
{
	...
	if ($set == 'normal')
	{
		$select_columns = '
			...
			mem.buddy_list, mg.online_color AS member_group_color, IFNULL(mg.group_name, {string:blank_string}) AS member_group, mem.active_seeders, mem.active_leechers, mem.downloaded, mem.uploaded, mem.dlrate_max, mem.ulrate_max,
...
function loadMemberContext($user, $display_custom_fields = false)
{
	...
	$memberContext[$user] = array(
		...
		'active' => !empty($profile['active_leechers']) || !empty($profile['active_seeders']) ? array($profile['active_leechers'], $profile['active_seeders']) : array(0, 0),		
	);
