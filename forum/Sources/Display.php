...
// Check if spellchecking is both enabled and actually working. (for quick reply.)
	...
	$context['save_draft'] = $context['can_reply'] && allowedTo('post_draft') && !empty($modSettings['masterSaveDrafts']);
	$context['save_draft_auto'] = $context['save_draft'] && allowedTo('draft_autosave') && !empty($modSettings['masterAutoSaveDrafts']);
...
function prepareDisplayContext($reset = false)
{
	...
	if (!empty($options['display_quick_reply']) && !$context['user']['is_guest'] && empty($context['arantor_menu']))
		$output['member']['link'] = '<span style="cursor: pointer;' . (!empty($output['member']['user_color']) ? ' color: #' . $output['member']['user_color'] : '') . '" onclick="nicknameToReply(\'[b]' . (!empty($output['member']['user_color']) ? '[color=#' . $output['member']['user_color'] . ']' : (!empty($output['member']['group_color']) ? '[color=' . $output['member']['group_color'] . ']' : '')) . $output['member']['name'] . (!empty($output['member']['user_color']) || !empty($output['member']['group_color']) ? '[/color]' : '') . '[/b], \');" title="' . $txt['nickname_hint'] . '">' . (!empty($modSettings['MemberColorLink']) && !empty($output['member']['colored_name']) ? $output['member']['colored_name'] : $output['member']['name']) . '</span>';
