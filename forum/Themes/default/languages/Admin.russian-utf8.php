...
$txt['admin_censored_desc'] = 'В силу того, что форум является общественным, возможно, есть некоторые слова, которые Вы желали бы запретить к использованию пользователями форума. Ниже можно ввести эти слова.<br />Пустое поле справа удалит слово.';
...
$txt['admin_main_welcome'] = '<div style="margin-top: 1ex;">Это Ваш &laquo;%1$s&raquo;. Здесь можно управлять настройками сервера, обслуживать форум, просматривать логи, устанавливать пакеты модификаций, изменять темы оформления и многое другое. При возникновении проблем, пожалуйста, посетите страницу &laquo;Поддержка и благодарности&raquo;. Если эта информация Вам не помогла, попробуйте обратиться со своей проблемой на <a href="http://www.simplemachines.org/community/index.php" target="_blank">официальный форум поддержки</a>. Вы также можете получить подсказку, нажав на значок <img src="' . $settings['images_url'] . '/helptopics.gif" alt="%2$s" title="%3$s" /> рядом с соответствующей опцией.</div>';
...
$txt['admin_newsletter_send'] = 'Вы можете отправить письмо любому пользователю с этой страницы. Email адреса выбранных пользователей будут показаны ниже, но Вы можете добавлять или удалять их по своему желанию. Убедитесь в том, что каждый адрес разделен, как это показано на примере: \'адрес1; адрес2\'.';
...
$txt['attachments_desc'] = 'Здесь можно администрировать вложенные файлы. Вы можете удалить вложения по размеру, по дате публикации. Статистика по вложениям предоставлена ниже.';
...
$txt['attachment_pruning_warning'] = 'Вы уверены в том, что хотите удалить эти вложения?\\nДействие не может быть отменено!';
...
$txt['email_variables'] = 'В этом сообщении Вы можете использовать некоторые &quot;специальные&quot; переменные.  Нажмите <a href="' . $scripturl . '?action=helpadmin;help=emailmembers" onclick="return reqWin(this.href);" class="help">здесь</a> для просмотра подробной информации.';
...
$txt['ban_description'] = 'Здесь можно блокировать пользователей по IP адресу, имени хоста, имени пользователя или по email.';
...
$txt['maintain_info'] = 'Оптимизирование таблиц, создание резервных копий, проверка на ошибки и очистка форума — всё это осуществляется при помощи данной утилиты.';
...
$txt['repair_attachments_error_desc'] = 'В ходе обслуживания были найдены ошибки. Выделите те из них, которые Вы хотите исправить, и нажмите продолжить.';
...
$txt['news_mailing_desc'] = 'Здесь можно отправлять сообщения всем зарегистрированным пользователям. Вы можете настроить список рассылки или отправить сообщение всем. Полезно для рассылки важной информации и новостей.';
...
$txt['manageposts_bbc_settings_description'] = 'BB код может использоваться для оформления сообщений. Пример жирного выделения слова  \'лошадь\': Вы можете написать так: [b]лошадь[/b]. Все BB-теги обрамлены квадратными скобками (\'[\' и \']\').';
...
$txt['admin_news_select_email_desc'] = 'Email адреса, на которые отправляется сообщение, должны быть разделены точкой с запятой (адрес1; адрес2)';
