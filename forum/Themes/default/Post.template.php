...
function template_main()
{
	...
	{
		// Check for admin, moderator and first post. Show subject textbox.
		if ($modSettings['disableSubjectChange'] == 0 || ($context['is_first_post'] || $context['user']['is_admin'] || $context['user']['can_mod']))
			echo '
		...
		// User is a...user. Disable subject textbox and show the subject only.
		else
			echo '
							', $context['subject'] == '' ? '(No Subject)' : $context['subject'], ' <input type="hidden" name="subject"', $context['subject'] == '' ? '' : ' value="' . $context['subject'] . '"', ' tabindex="', $context['tabindex']++, '" size="80" maxlength="80" class="input_text" />';
	}
	...
	{
		if(!empty($modSettings['keycaptcha_is_active']) && ($modSettings['keycaptcha_is_active'] == 1 && !empty($modSettings['keycaptcha_js_code']) && !empty($modSettings['keycaptcha_private_key'])))
		{
			echo '
				<div class="post_verification">
					...
					<td width="100%">';
						...
						$kc_o = new KeyCAPTCHA_CLASS($modSettings['keycaptcha_private_key'], $modSettings['keycaptcha_js_code']);
			...
			if ($context['can_quote'])
			{
				echo '
					<ul class="reset smalltext quickbuttons" id="msg_', $post['id'], '_quote">
						', !$context['user']['is_guest'] ? '<li class="reply_button"><a href="javascript:void(0);" onclick="nicknameToReply(\'[b]' . $post['poster'] . '[/b], \');" title="' . $txt['nickname_hint'] . '">' . $txt['nickname'] . '</a></li>' : '', '
