...
<html xmlns="http://www.w3.org/1999/xhtml"', $context['right_to_left'] ? ' dir="rtl"' : '', '>
	...
	echo'
		...
		$url_textonly = $scripturl . '?action=printpage;topic=' . $topic . '.0';
		$url_images = $url_textonly . ';images';
		if (isset($_REQUEST['images']))
			echo '
			<a href="', $url_textonly, '">', $txt['print_textonly'], '</a> | <strong><a href="', $url_images, '">', $txt['print_withimages'], '</a></strong>';
		else
			echo '
			<strong><a href="', $url_textonly, '">', $txt['print_textonly'], '</a></strong> | <a href="', $url_images, '">', $txt['print_withimages'], '</a>';
...
function template_main()
{
	...
	foreach ($context['posts'] as $post)
	{
		...
		if (isset($_REQUEST['images']) && !empty($context['printattach'][$post['id_msg']]))
		{
			echo '<br /><hr />';
			foreach ($context['printattach'][$post['id_msg']] as $attach)
				echo '
					<img width="' . $attach['width'] . '" height="' . $attach['height'] . '" alt="" src="', $scripturl . '?action=dlattach;topic=' . $post['topic'] . '.0;attach=' . $attach['id_attach'] . '" />';
		}
		echo '
			</dd>';
	}
