...
function smf_main()
{
    ...
    elseif ((empty($modSettings['allow_guestAccess'])) && $user_info['is_guest'] && (!isset($_REQUEST['action']) || !in_array($_REQUEST['action'], array('push_content_check', 'user_subscription', 'get_contact', 'set_api_key', 'reset_push_slug', 'sync_user', 'prefetch_account', 'update_password', 'forget_password', 'sign_in', 'coppa', 'login', 'login2', 'register', 'register2', 'reminder', 'activate', 'help', 'smstats', 'mailq', 'verificationcode', 'openidreturn',))))
    {
    ...
    if (in_array($_REQUEST['action'], $local_action)) {
        if(file_exists('include/' . $actionArray[$_REQUEST['action']][0])){
