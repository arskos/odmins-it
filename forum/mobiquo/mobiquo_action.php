...
function action_sign_in()
{
    ...
    $sso = new TTSSOBase(new TTSSOForum());
    $data = array(
        'token' => $_POST['token'],
        'code' => $_POST['code'],
        'email' => $_POST['email'],
...
function action_get_contact()
{
    ...
    if (!preg_match('/[A-Z0-9]{32}/', $mobi_api_key) || empty($code)){
        $result['result_text'] = 'no find api_key';
    ...
    {
        $result['result'] = true;
        $users = preg_split('/,/', $user_id);
        loadMemberData($users, false, 'profile');
        ...
        foreach ($users as $user){
            ...
            {
                $result['users'][] = array(
                    'user_id'       => $profile['id_member'],
                    'display_name'  => basic_clean($profile['real_name']),
                    'enc_email'     => base64_encode(tt_encrypt(trim($profile['email_address']), $mobi_api_key)),
                    'allow_email'   => $profile['notify_announcements'],
                    'language'      => $profile['lngfile'],
...
function action_sync_user(){
    ...
    if($response === true)
    {
        ...
        $request_members = $smcFunc['db_query']('',
        "SELECT id_member as uid, real_name as username, email_address as user_email,notify_announcements as allow_email,lngfile as language, date_registered, posts, last_login 
        FROM {db_prefix}members
        WHERE is_activated = 1 and id_member > {int:start} ORDER BY id_member ASC LIMIT {int:limit}",
        array(
            'start'=>$start,
            'limit'=>$limit,
        ));
        while ($row = $smcFunc['db_fetch_assoc']($request_members))
        {
            ...
            $user['encrypt_email'] = base64_encode(tt_encrypt(trim($row['user_email']), $mobi_api_key));
            $user['allow_email'] = $row['allow_email'];
            $user['language'] = !empty($row['language']) ? $row['language'] : $context['user']['language'];
            $user['reg_date'] = $row['date_registered'];
            $user['post_num'] = $row['posts'];
