...
foreach($razdel as $key => $value)
{
	...
	while ($main=mysql_fetch_array($my))
	{
		$value_temp="99999".$value;
 		if ((mysql_query("UPDATE `admins_page` SET ids_podrazdel='$value_temp' WHERE ids_razdel='$ids_razdel' && ids_podrazdel='$key'"))===false)
			{echo mysql_error();}
	}
}
...
while ($main=mysql_fetch_array($my))
{
	$temp_ids=substr($main['ids_podrazdel'],0,5);
	$ids=$main['ids_podrazdel'];
	if ($temp_ids==="99999")
	{
		$real_ids=substr($main['ids_podrazdel'],5);
		if ((mysql_query("UPDATE `admins_page` SET ids_podrazdel='$real_ids' WHERE ids_razdel='$ids_razdel' && ids_podrazdel='$ids'"))===false)
			{echo mysql_error();}
	}
}
