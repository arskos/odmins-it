<form action="admin.php" method="post" name="mails_user_show">
<table width="100%"  border="0" cellspacing="1" cellpadding="3">
<tr class="blue1">
	<td width="12%" height="21" background="imgadm/mm_top2.gif" class="blue1">Дата</td>
	<td width="29%" background="imgadm/mm_top2.gif" class="blue1">Email</td>
	<td width="30%" background="imgadm/mm_top2.gif" class="blue1">Комментарий</td>
...
<?
$my=mysql_query("SELECT * FROM `mail_user_tasman` ORDER BY data DESC;");
...
$my=mysql_query("SELECT * FROM `mail_user_tasman` ORDER BY data DESC LIMIT $mails_kol,10;");
$i=0;
while ($main=mysql_fetch_array($my))
{
...
<?} else {?><tr onMouseOver="this.bgColor = '#EFF3F7'" onMouseOut ="this.bgColor = '#F8FAFC'" bgcolor="F8FAFC"><?}?>
		...
		<td class="blue1"><div align="justify"><span class="ntxt"><?echo $main['mails_email'];?></td>
		<td class="blue1"><div align="justify"><span class="ntxt"><?echo $main['mails_comment'];?></td>
		<td><div align="center"><a href="admin.php?mails=yes&mails_user_edit=yes&ids=<?echo $main['id'];?>"><img src="imgadm/b_edit.gif" alt="Редактировать" width="20" height="20" border="0"></a></div></td>
		<td ><div align="center"><a href="#" onClick=answer_mails_user(<?echo $main['id'];?>)><img src="imgadm/b_del.gif" alt="Удалить" width="20" height="20" border="0"></a></div></td>
		<td ><div align="center">
			<input name="mails_user_check[<? echo $i;?>]" type="checkbox" class="fld" value="<?echo $main['id'];?>"></div>
		</td>
	</tr>
<?
$i++;
...
<tr>
	<td height="21" colspan="5" bgcolor="EFEFEF" class="blue1">
		...
		<tr>
			<td height="21" width="22"><a href="#" onClick=querys_mails_user()><img src="imgadm/b_del.gif" width="20" height="20" border="0"></a></td>
			<td width="573"><a href="#" class="blue1" onClick=querys_mails_user()>удалить все выделенные </a></td>
			...
			<input type="hidden" name="mails_user_del_all" value="yes">
...
if ($kol_rows>10)
{
	...
	{
		echo "<td>";
		echo "<a href='admin.php?mails=yes&mails_user_show=yes&mails_kol=".($start-$del)."' class='pg'>&lt&lt</a>";
		echo "</td>";
	}
	for ($i=$start;$i<=$fin;$i++)
	{
		...
		else {echo "<a href='admin.php?mails=yes&mails_user_show=yes&mails_kol=".$i."' class='pg'>".$i."</a>";}
		echo "</td>";
	}
	if (($kol_rows>($del*10)) and ((($start+$del))<=$kol_str))
	{
		echo "<td>";
		echo "<a href='admin.php?mails=yes&mails_user_show=yes&mails_kol=".($fin+1)."' class='pg'>&gt&gt</a>";
		echo "</td>";
	}
}
?>
