...
$vpage_ids=$_GET['vpage_ids'];
if (empty($vpage_ids)){$vpage_ids=$_POST['vpage_ids'];}
$my=mysql_query("SELECT * FROM `comperence_vpage` WHERE id=$vpage_ids;");
$main=mysql_fetch_array($my);
if (empty($vpage_name)){$vpage_name=$main['name'];}
if (empty($vpage_comment)){$vpage_comment=$main['comment'];}
$vpage_kod=$main['kod'];
if (empty($postedValue)) {$postedValue=$vpage_kod;}
?>
...
<tr>
	<td>
		...
		<input type="hidden" name="vpage_edit_vis" value="yes">
		<input type="hidden" name="vpage_post" value="yes">
		<input type="hidden" name="vpage_ids" value="<?echo $vpage_ids;?>">
</table>
</form>
<form name="vpage_add_save" action="admin.php" method="post">
	<input type="hidden" name="vpage_edit_ok" value="yes">
	<input type="hidden" name="vpage_post" value="yes">
