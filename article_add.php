...
if ($_REQUEST['tt_pass'])
{
	...
	if (empty($err))
	{
		...
		$e_mail=$main['email'];
		$tl="от ".$tname."<br>Тема: ".$ttema."<br>Анонс: ".$tanons."<br>Текст: ".$ttxt."\n";
		send_mime_mail($tname,
               ...
               $tl);
		$ok="yes";
		$max=mysql_result((mysql_query("SELECT MAX(id) FROM `admins_article`;")),0);
		$max=$max+10;
		mysql_query("INSERT INTO `admins_article` VALUES ('$max','$ttema','$tanons','$ttxt','','','','0');");
	}
}
...
if (($ok==="yes") && (empty($err)))
{
	echo "<span class='txt'><strong>Ваше статья отправлена на рассмотрениии</strong></span>";
	session_destroy();
	$tt_pass="";
	$tname="";
	$temail="";
...
{
?>
<form name="send_mes" method="post" action="/article_add/" onSubmit="return checkData2();">
...
<tr>
	<td height="25"><div align="right">Тема</div></td>
	<td><input name="ttema" type="text" class="txt" id="ttema" size="30" maxlength="30" style="width:90%" value="<?if (!empty($ttema)){echo $ttema;}?>"></td>
</tr>
<tr>
	<td valign="top"><div align="right">Анонс</div></td>
	<td><textarea name="tanons" cols="45" rows="2" class="tanons" id="tanons" style="width:90%"><?if (!empty($tanons)){echo $tanons;}?></textarea></td>
</tr>
<tr>
